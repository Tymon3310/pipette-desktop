      ...
      <div className="flex flex-col items-center gap-7">
        <div className="relative flex h-14 w-14 items-center justify-center" aria-hidden="true">
          ...
          <svg
            ...
            viewBox="0 0 24 24"
            fill="none"
            className="absolute text-content-muted opacity-50"
          >
            <rect
              x="2"
              y="6"
        ...
        <div className="flex min-h-[7rem] max-w-xs flex-col items-center justify-center gap-2">
          ...
          {deviceId && (
            <p className="font-mono text-[11px] tracking-wide text-content-muted">{deviceId}</p>
          )}
          <p className="h-4 text-xs text-content-muted">
          ...
          {(syncProgress?.status === 'error' || syncProgress?.status === 'partial') &&
            syncProgress?.message && (
              <div
                className={`text-sm ${syncProgress.status === 'error' ? 'text-danger' : 'text-warning'}`}
              >
