...
function validateTypingTestConfig(raw: unknown): TypingTestConfig | undefined {
  ...
  switch (obj.mode) {
    case 'words':
      if (!isFinitePositiveInt(obj.wordCount) || !hasBooleanFields(obj, 'punctuation', 'numbers'))
        return undefined
      return {
        mode: 'words',
        wordCount: obj.wordCount,
        punctuation: obj.punctuation as boolean,
        numbers: obj.numbers as boolean,
      }
    case 'time':
      if (!isFinitePositiveInt(obj.duration) || !hasBooleanFields(obj, 'punctuation', 'numbers'))
        return undefined
      return {
        mode: 'time',
    ...
    case 'quote':
      if (typeof obj.quoteLength !== 'string' || !VALID_QUOTE_LENGTHS.has(obj.quoteLength))
        return undefined
...
function validateIpcPrefs(
  data: {
    ...
    autoAdvance: boolean
    layerPanelOpen?: boolean
    basicViewType?: string
    splitKeyMode?: string
    quickSelect?: boolean
    layerNames?: string[]
    typingTestResults?: TypingTestResult[]
    typingTestConfig?: unknown
    typingTestLanguage?: unknown
  } | null,
  ...
  const layout =
    typeof data.keyboardLayout === 'string' && LAYOUT_ID_SET.has(data.keyboardLayout)
      ? data.keyboardLayout
      : null
  ...
  const layerPanelOpen =
    typeof data.layerPanelOpen === 'boolean' ? data.layerPanelOpen : defaultLayerPanelOpen
  const rawBasicView =
    typeof data.basicViewType === 'string'
      ? (LEGACY_BASIC_VIEW_MAP[data.basicViewType] ?? data.basicViewType)
      : null
  const basicViewType =
    rawBasicView !== null && VALID_BASIC_VIEW_TYPES.has(rawBasicView)
      ? (rawBasicView as BasicViewType)
      : defaultBasicViewType
  const splitKeyMode =
    typeof data.splitKeyMode === 'string' && VALID_SPLIT_KEY_MODES.has(data.splitKeyMode)
...
export function useDevicePrefs(): UseDevicePrefsReturn {
  ...
  const [layerPanelOpen, updateLayerPanelOpen, layerPanelOpenRef] =
    useStateRef<boolean>(defaultLayerPanelOpen)
  const [basicViewType, updateBasicViewType, basicViewTypeRef] =
    useStateRef<BasicViewType>(defaultBasicViewType)
  const [splitKeyMode, updateSplitKeyMode, splitKeyModeRef] =
    useStateRef<SplitKeyMode>(defaultSplitKeyMode)
  ...
  const [typingTestResults, updateTypingTestResults, typingTestResultsRef] = useStateRef<
    TypingTestResult[]
  >([])
  const [typingTestConfig, updateTypingTestConfig, typingTestConfigRef] = useStateRef<
    TypingTestConfig | undefined
  >(undefined)
  const [typingTestLanguage, updateTypingTestLanguage, typingTestLanguageRef] = useStateRef<
    string | undefined
  >(undefined)
  ...
  const setLayerPanelOpen = useCallback(
    (open: boolean) => {
  ...
  const setBasicViewType = useCallback(
    (type: BasicViewType) => {
  ...
  const setSplitKeyMode = useCallback(
    (mode: SplitKeyMode) => {
  ...
  const setLayerNames = useCallback(
    (names: string[]) => {
  ...
  const addTypingTestResult = useCallback(
    (result: TypingTestResult) => {
      const updated = trimResults(
        [result, ...typingTestResultsRef.current],
        MAX_TYPING_TEST_RESULTS,
      )
  ...
  const setTypingTestConfig = useCallback(
    (cfg: TypingTestConfig) => {
  ...
  const setTypingTestLanguage = useCallback(
    (lang: string) => {
  ...
  const setDefaultAutoAdvance = useCallback(
    (enabled: boolean) => {
  ...
  const setDefaultLayerPanelOpen = useCallback(
    (open: boolean) => {
  ...
  const setDefaultBasicViewType = useCallback(
    (type: BasicViewType) => {
  ...
  const setDefaultSplitKeyMode = useCallback(
    (mode: SplitKeyMode) => {
  ...
  const setDefaultQuickSelect = useCallback(
    (enabled: boolean) => {
  ...
  const applyDevicePrefs = useCallback(
    async (uid: string) => {
      uidRef.current = uid
  ...
  const remapLabel = useCallback((qmkId: string): string => remapKeycode(qmkId, layout), [layout])
