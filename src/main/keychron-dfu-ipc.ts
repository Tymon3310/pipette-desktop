...
export function setupKeychronDfuIpc(): void {
  secureHandle(
    IpcChannels.KEYCHRON_DFU_FLASH,
    async (event: Electron.IpcMainInvokeEvent, firmwareData: unknown) => {
      ...
      try {
        ...
        return {
          success: false,
          error: `Failed to write temp firmware file: ${(err as Error).message}`,
        }
      }
      ...
      return new Promise((resolve) => {
        ...
        event.sender.send(IpcChannels.KEYCHRON_DFU_OUTPUT, {
          log: 'Waiting for DFU device (up to 60s)...',
          progress: 0,
        })
        ...
        const checkDfuDevice = () => {
          if (Date.now() > timeout) {
            event.sender.send(IpcChannels.KEYCHRON_DFU_OUTPUT, {
              log: 'Timeout waiting for DFU device',
            })
          ...
          ls.on('error', (err) => {
            event.sender.send(IpcChannels.KEYCHRON_DFU_OUTPUT, {
              log: `Error running dfu-util: ${err.message}`,
            })
            fs.unlink(firmwarePath).catch(() => {}) // Cleanup
        ...
        const startFlashing = () => {
          ...
          event.sender.send(IpcChannels.KEYCHRON_DFU_OUTPUT, {
            log: 'DFU device found. Starting flash...',
            progress: 0.05,
          })
            ...
            for (const line of lines) {
              ...
              if (match) {
                ...
                if (phase === 'Erase') {
                  ...
                  overallProgress = 0.5 + (pct * 0.5) / 100
                }
          ...
          activeDfuProcess?.on('close', (code) => {
            ...
            if (code === 0) {
              ...
              event.sender.send(IpcChannels.KEYCHRON_DFU_OUTPUT, {
                log: `dfu-util exited with code ${code}`,
              })
          ...
          activeDfuProcess?.on('error', (err) => {
            ...
            event.sender.send(IpcChannels.KEYCHRON_DFU_OUTPUT, {
              log: `Error spawning dfu-util: ${err.message}`,
            })
